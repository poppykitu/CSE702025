  ...
  return (
    <div style={{ padding: 24, background: 'var(--color-bg)', minHeight: '100%' }}>
      ...
      <div style={bentoGridStyle}>
        ...
        <div style={{ ...cardBaseStyle, gridColumn: screens.lg ? 'span 2' : 'span 1', gridRow: 'span 1', position: 'relative', background: 'linear-gradient(135deg, #1E293B 0%, #0F172A 100%)', color: '#fff' }}>
          <div style={{ padding: 24, display: 'flex', alignItems: 'center', gap: 20, h: '100%' }}>
            <Avatar size={80} style={{ backgroundColor: 'var(--color-primary)', border: '4px solid rgba(255,255,255,0.1)' }}>
              {profile?.full_name?.charAt(0)}
            </Avatar>
            <div>
              <h2 style={{ color: '#fff', margin: 0, fontSize: 22, fontWeight: 800 }}>{profile?.full_name}</h2>
              <p style={{ color: 'rgba(255,255,255,0.6)', margin: '4px 0 12px' }}>{ROLE_LABELS[role]}</p>
              <Space>
                <Tag color="success" icon={<CheckCircleOutlined />}>Đã xác thực</Tag>
                <Tag color="processing" icon={<ThunderboltOutlined />}>Online</Tag>
              </Space>
            </div>
          </div>
          <div style={{ position: 'absolute', right: -20, bottom: -20, opacity: 0.1 }}>
             <TeamOutlined style={{ fontSize: 120 }} />
          </div>
        </div>
        ...
        <div style={{ ...cardBaseStyle, gridColumn: 'span 1', gridRow: (screens.lg || screens.md) ? 'span 2' : 'span 1' }}>
           ...
           <div style={{ flex: 1, padding: '12px 24px', overflowY: 'auto' }}>
             {[1, 2, 3, 4, 5].map(i => (
               <div key={i} style={{ padding: '10px 0', borderBottom: i < 5 ? '1px solid var(--color-border-subtle)' : 'none' }}>
                 <div style={{ fontSize: 13, fontWeight: 600 }}>Cập nhật hồ sơ</div>
                 <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Quản trị viên • 12 phút trước</div>
               </div>
             ))}
           </div>
        </div>
        ...
        <div style={{ ...cardBaseStyle, gridColumn: 'span 1', background: 'var(--color-primary-bg)', border: '1px solid var(--color-primary-border)' }}>
